...
public class FrontController extends HttpServlet {
   ...
   Map<String, Command> list = null; //최상위타입 이전인 logincontroller와 joingcontroller의 부모로 바꿔줌

   // 최초로 요청 들어왔을 때 딱 한번만 실행
   public void init(ServletConfig config) throws ServletException {
      // 여기로 들어올 수 있는 요청 경로를 미리 리스트로 만들어놓기!
      // map(순서가 없는 구조) 사용! (key-value)
      // -> (key(요청경로-join.do/login.do -> String)
      //      -value(그 기능을 수행해줄 컨트롤러 객체))
      list = new HashMap<>();
   ...
   protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
      ...
      String subUri = uri.substring(path.length()); // 잘라내기
